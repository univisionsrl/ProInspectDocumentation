...
| CAD fileDisplayed only when CAD (Closed ROI) geometry is selected. | CAD file name. |
...
| Second thresholdDisplayed only when threshold mode "Standard" and polarity "Dark and white objects" are selected. | Mode standard: If polarity is Dark objects, pixels with grey-scale value below the threshold are considered as foreground, while all pixels with value above the threshold are assigned as background pixels. The opposite for White objects. (default = 0 | min = 0 | max = 255) |
